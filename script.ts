interface Movie {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const movieName = document.querySelector<HTMLInputElement>(".search__input");
    const searchButton = document.querySelector<HTMLButtonElement>("#search-btn");
    const resultWrapper = document.querySelector<HTMLDivElement>("#result");
    const paginationWrapper = document.querySelector<HTMLUListElement>("#pagination");
    const form = document.querySelector<HTMLFormElement>("#search__form--film");
    const key = "abfdbba4";
    ...
    function searchFilmHandler(page = 1): void {
        if (isLoading) {
            return;
          }
